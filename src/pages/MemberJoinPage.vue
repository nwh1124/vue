    ...
    <div class="container mx-auto">
      <div class="px-6 py-6 bg-white rounded-lg shadow-md">
        <form v-if="globalShare.isLogined == false" v-on:submit.prevent="checkAndJoin">
          ...
          <FormRow title="전화번호">
            <input ref="phoneNumberElRef" class="form-row-input" type="tel" placeholder="전화번호를 입력해주세요.">
          </FormRow>
          <FormRow title="이메일">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function checkAndJoin() {
      ...
      // 휴대전화번호 체크
      if ( phoneNumberElRef.value == null ) {
        return;
      }
      const phoneNumberEl = phoneNumberElRef.value;
      phoneNumberEl.value = phoneNumberEl.value.trim();
      if ( phoneNumberEl.value.length == 0 ) {
